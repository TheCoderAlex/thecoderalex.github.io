...
* [Object](#object): 概述, 重写toString 和 equals方法
* [System](#system): 概述, arraycopy(…), currentTimeMillis(), exit(int status), gc()
* [Date](#date): 概述, 构造方法, setTime(long time), getTime()
* [DateFormat](#dateformat): 概述, 格式化format方法, 解析parse方法, 构造方法, 指定模式字母
* [Calendar](#calendar): 概述, 创建, 获取get方法, 修改set方法, 添加add方法
* [包装类](#包装类): 概述, 组成, Integer的构造和转换
* [自动装箱和拆箱](#自动装箱和拆箱): 概述, 装拆示例
* [正则表达式](#正则表达式): 概述, 常用规则, qq号码判断示例
<!--GFM-TOC -->
...
- ### 自动装箱和拆箱
  - 自动装箱和拆箱：
    ...
    - 装箱：基本数据类型转换为包装类对象
    - 拆箱：包装类对象转换为基本数据类型
    - java虚拟机帮我们做了装箱和拆箱的动作，我们只需要做简单的代码即可。 
  - 自动装箱：
    - Integer i = 10;
  ...
  ```java
	ArrayList list = new ArrayList();//集合存储对象
...
- ### 示例：
  ...
  ```java
	String s3 = "0123456";
	Boolean flag = s3.matches("[1-9][0-9]{4,15}");//巧用正则表达式，免去复杂的if-else判断
...
> 作者: alextang  
> URL: https://alextang.top/articles/2024/03/java%E5%B8%B8%E7%94%A8api/  
